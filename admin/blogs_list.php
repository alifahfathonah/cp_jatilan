...
<div class="container-fluid">
                        ...
                        <div class="row-fluid">
                                ...
                                <div class="block">
                                    <div class="navbar navbar-inner block-header">
                                        <div class="muted pull-left">List Blogs</div>
                                    </div>
        ...
        if($_SESSION['level'] == 1){ ?>
            ...
            <?php
            $no = 1;
            $sql = $mysqli->query("SELECT b.id, b.title, b.profile_id, b.content, b.image, c.name, p.nick_name FROM jtl_blogs b JOIN jtl_category_blog c ON b.category_blog = c.id JOIN jtl_users u ON b.user_id = u.id JOIN jtl_profiles p ON b.profile_id = p.id");
            while($row = $sql->fetch_array()){ ?>
                <tr>
                    ...
                    <td><img src="<?php echo $row['image']; ?>" class="thumbnail" style="width: 200px; height: 100px"> </td>
                    <td><?php echo $row['name']; ?></td>
                    <td><a href="biography.php?id=<?php echo $row['profile_id'] ?>"><?php echo $row['nick_name']; ?></a> </td>
                    <td>
                        <a href="blogs_detail.php?id=<?php echo $row['id']?>" class="btn btn-mini" title="Details">
                            <i class="icon-eye-open"></i>
                        </a>
                        <a href="blogs_edit.php?id=<?php echo $row['id'] ?>" class="btn btn-mini btn-warning" title="Edit">
                            <i class="icon-pencil icon-white"></i>
                        </a>
                        <a href="blogs_delete.php?id=<?php echo $row['id'] ?>" class="btn btn-mini btn-danger" title="Delete" onclick="return confirm('Anda yakin ingin hapus data tersebut ?')">
        ...
        <?php }elseif($_SESSION['level'] == 2 ){ ?>
            ...
            <?php
            $no = 1;
            $sql = $mysqli->query("SELECT b.title, b.content, b.profile_id b.image FROM jtl_blogs b JOIN jtl_category_blog c ON b.category_blog = c.id JOIN jtl_users u ON b.user_id = u.id JOIN jtl_profiles p ON b.profile_id = p.id WHERE user_id='".$_SESSION['id']."'");
            while($row = $sql->fetch_array()){ ?>
                <tr>
                    <td><?php echo $no; ?></td>
                    <td>
