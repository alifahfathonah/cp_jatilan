...
<div class="container-fluid">
        ...
        if($_SESSION['level'] == 1){ ?>
            ...
            <?php
            $no = 1;
            $sql = $mysqli->query("SELECT a.id, a.name, a.image, a.profile_id, a.descriptions, a.date_created, p.nick_name FROM jtl_accessories a JOIN jtl_users u ON a.user_id = u.id JOIN jtl_profiles p ON a.profile_id = p.id");
            while($row = $sql->fetch_array()){ ?>
                <tr>
                    ...
                    <td>
                        <a href="acc_jatilan_detail.php?id=<?php echo $row['id']?>" class="btn btn-mini" title="Details">
                            <i class="icon-eye-open"></i>
                        </a>
                        <a href="acc_jatilan_edit.php?id=<?php echo $row['id'] ?>" class="btn btn-mini btn-warning" title="Edit">
                            <i class="icon-pencil icon-white"></i>
                        </a>
                        <a href="acc_jatilan_delete.php?id=<?php echo $row['id'] ?>" class="btn btn-mini btn-danger" title="Delete" onclick="return confirm('Anda yakin ingin hapus data tersebut ?')">
        ...
        <?php }elseif($_SESSION['level'] == 2 ){ ?>
            ...
            <?php
            $no = 1;
            $sql = $mysqli->query("SELECT a.id, a.name, a.image, a.descriptions, a.date_created, p.nick_name FROM jtl_accessories a JOIN jtl_users u ON a.user_id = u.id JOIN jtl_profiles p ON a.profile_id = p.id user_id='".$_SESSION['id']."'");
            while($row = $sql->fetch_array()){ ?>
                <tr>

                    <td><?php echo $no; ?></td>
                    <td>
                        <img src="<?php echo $row['gambar_gallery']; ?>" class="thumbnail" style="width: 200px; height: 100px">
                        <p style="font-size: 11px; color: #c09853"><i><?php echo $row['date_created']; ?></i></p>
                    </td>
